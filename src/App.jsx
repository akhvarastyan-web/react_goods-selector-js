...
export const App = () => {
  ...
  <main className="section container">
    {!selectedGood ? <h1 className="title is-flex is-align-items-center">No goods selected</h1>
    ...
    <table className="table">
      <tbody>
        ...
        className={`${selectedGood === x ? 'has-background-success-light' : ''}`}>
          <td>
            <button data-cy={selectedGood === x ? "RemoveButton" :"AddButton"} type="button"
            className={`${selectedGood === x ? 'button is-info' : 'button'}`}
            onClick={() => {setGoods(selectedGood === x ? '' : x)}}>
              {selectedGood === x ? '-' : '+'}
            </button>
          </td>
